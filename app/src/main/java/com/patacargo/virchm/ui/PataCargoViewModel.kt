package com.patacargo.virchm.ui
    ...
    fun connectMercadoPagoAccount(userId: String, email: String, onResult: (Boolean, String?) -> Unit) {
        viewModelScope.launch(Dispatchers.IO) {
            try {
                ...
                val rawToken = com.patacargo.virchm.BuildConfig.MERCADO_PAGO_ACCESS_TOKEN
                ...
                val searchResponse = try {
                    com.patacargo.virchm.api.RetrofitClient.mercadoPagoApi.searchCustomer(authHeader, email.trim().lowercase())
                } catch (e: Exception) {
                    null
                }
                ...
                if (searchResponse != null && searchResponse.isSuccessful) {
                    ...
                    if (results != null && results.isNotEmpty()) {
                        ...
                        val createResponse = com.patacargo.virchm.api.RetrofitClient.mercadoPagoApi.createCustomer(authHeader, createBody)
                    ...
                    val createResponse = try {
                        com.patacargo.virchm.api.RetrofitClient.mercadoPagoApi.createCustomer(authHeader, createBody)
                    } catch (e: Exception) {
                        null
                    }
    ...
    fun generateMercadoPagoPreference(
        ...
        viewModelScope.launch(Dispatchers.IO) {
            try {
                ...
                val response = com.patacargo.virchm.api.RetrofitClient.mercadoPagoApi.createPreference(authHeader, body)
    ...
    fun verifyMercadoPagoPayment(shipmentId: Int, onResult: (Boolean, String?) -> Unit) {
        viewModelScope.launch(Dispatchers.IO) {
            try {
                ...
                val response = com.patacargo.virchm.api.RetrofitClient.mercadoPagoApi.searchPayments(authHeader, externalReference)
